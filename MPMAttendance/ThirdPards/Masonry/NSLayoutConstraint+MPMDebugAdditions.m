//
//  NSLayoutConstraint+MPMDebugAdditions.m
//  MPMMasonry
//
...
#import "NSLayoutConstraint+MPMDebugAdditions.h"
#import "MPMConstraint.h"
#import "MPMLayoutConstraint.h"

@implementation NSLayoutConstraint (MPMDebugAdditions)
...
+ (NSDictionary *)layoutPriorityDescriptionsByValue {
    ...
    dispatch_once(&once, ^{
        ...
        descriptionMap = @{
            @(MPMLayoutPriorityDefaultHigh)      : @"high",
            @(MPMLayoutPriorityDefaultLow)       : @"low",
            @(MPMLayoutPriorityDefaultMedium)    : @"medium",
            @(MPMLayoutPriorityRequired)         : @"required",
            @(MPMLayoutPriorityFittingSizeLevel) : @"fitting size",
        };
#elif TARGET_OS_MAC
        descriptionMap = @{
            @(MPMLayoutPriorityDefaultHigh)                 : @"high",
            @(MPMLayoutPriorityDragThatCanResizeWindow)     : @"drag can resize window",
            @(MPMLayoutPriorityDefaultMedium)               : @"medium",
            @(MPMLayoutPriorityWindowSizeStayPut)           : @"window size stay put",
            @(MPMLayoutPriorityDragThatCannotResizeWindow)  : @"drag cannot resize window",
            @(MPMLayoutPriorityDefaultLow)                  : @"low",
            @(MPMLayoutPriorityFittingSizeCompression)      : @"fitting size",
            @(MPMLayoutPriorityRequired)                    : @"required",
        };
#endif
    });
...
+ (NSString *)descriptionForObject:(id)obj {
    if ([obj respondsToSelector:@selector(mpm_key)] && [obj mpm_key]) {
        return [NSString stringWithFormat:@"%@:%@", [obj class], [obj mpm_key]];
    }
...
- (NSString *)description {
    ...
    if (self.priority != MPMLayoutPriorityRequired) {
