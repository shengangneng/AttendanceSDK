...
#error MPMProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "MPMProgressHUD.h"
#import "MPMIndefiniteAnimatedView.h"
#import "MPMProgressAnimatedView.h"
#import "MPMRadialGradientLayer.h"

NSString * const MPMProgressHUDDidReceiveTouchEventNotification = @"MPMProgressHUDDidReceiveTouchEventNotification";
NSString * const MPMProgressHUDDidTouchDownInsideNotification = @"MPMProgressHUDDidTouchDownInsideNotification";
NSString * const MPMProgressHUDWillDisappearNotification = @"MPMProgressHUDWillDisappearNotification";
NSString * const MPMProgressHUDDidDisappearNotification = @"MPMProgressHUDDidDisappearNotification";
NSString * const MPMProgressHUDWillAppearNotification = @"MPMProgressHUDWillAppearNotification";
NSString * const MPMProgressHUDDidAppearNotification = @"MPMProgressHUDDidAppearNotification";

NSString * const MPMProgressHUDStatusUserInfoKey = @"MPMProgressHUDStatusUserInfoKey";

static const CGFloat MPMProgressHUDParallaxDepthPoints = 10.0f;
static const CGFloat MPMProgressHUDUndefinedProgress = -1;
static const CGFloat MPMProgressHUDDefaultAnimationDuration = 0.15f;
static const CGFloat MPMProgressHUDVerticalSpacing = 12.0f;
static const CGFloat MPMProgressHUDHorizontalSpacing = 12.0f;
static const CGFloat MPMProgressHUDLabelSpacing = 8.0f;
...
@property (nonatomic, strong) MPMRadialGradientLayer *backgroundRadialGradientLayer;
@property (nonatomic, strong) UIVisualEffectView *hudView;
@property (nonatomic, strong) UILabel *statusLabel;
@property (nonatomic, strong) UIImageView *imageView;

@property (nonatomic, strong) UIView *indefiniteAnimatedView;
@property (nonatomic, strong) MPMProgressAnimatedView *ringView;
@property (nonatomic, strong) MPMProgressAnimatedView *backgroundRingView;
...
+ (MPMProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static MPMProgressHUD *sharedView;
#if !defined(MPM_APP_EXTENSIONS)
...
+ (void)setDefaultStyle:(MPMProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(MPMProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(MPMProgressHUDAnimationType)type {
...
+ (void)setForegroundColor:(UIColor*)color {
    ...
    [self setDefaultStyle:MPMProgressHUDStyleCustom];
}
...
+ (void)showWithMaskType:(MPMProgressHUDMaskType)maskType {
    MPMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status {
    [self showProgress:MPMProgressHUDUndefinedProgress status:status];
}

+ (void)showWithStatus:(NSString*)status maskType:(MPMProgressHUDMaskType)maskType {
    MPMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(MPMProgressHUDMaskType)maskType {
    MPMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(MPMProgressHUDMaskType)maskType {
    MPMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(MPMProgressHUDMaskType)maskType {
    MPMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(MPMProgressHUDMaskType)maskType {
    MPMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status maskType:(MPMProgressHUDMaskType)maskType {
    MPMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(MPMProgressHUDMaskType)maskType {
    MPMProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)dismissWithCompletion:(MPMProgressHUDDismissCompletion)completion {
...
+ (void)dismissWithDelay:(NSTimeInterval)delay completion:(MPMProgressHUDDismissCompletion)completion {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = MPMProgressHUDMaskTypeNone;
        _defaultStyle = MPMProgressHUDStyleLight;
        _defaultAnimationType = MPMProgressHUDAnimationTypeFlat;
        ...
        _fadeInAnimationDuration = MPMProgressHUDDefaultAnimationDuration;
        _fadeOutAnimationDuration = MPMProgressHUDDefaultAnimationDuration;
        ...
        self.accessibilityIdentifier = @"MPMProgressHUD";
...
- (void)updateHUDFrame {
    ...
    hudWidth = MPMProgressHUDHorizontalSpacing + MAX(labelWidth, contentWidth) + MPMProgressHUDHorizontalSpacing;
    
    // |-spacing-content-(labelSpacing-label-)spacing-|
    hudHeight = MPMProgressHUDVerticalSpacing + labelHeight + contentHeight + MPMProgressHUDVerticalSpacing;
    if(self.statusLabel.text && (imageUsed || progressUsed)){
        // Add spacing if both content and label are used
        hudHeight += MPMProgressHUDLabelSpacing;
    }
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(MPMProgressHUDVerticalSpacing, (self.minimumSize.height - contentHeight - MPMProgressHUDLabelSpacing - labelHeight) / 2.0f);
    ...
    if(self.progress != MPMProgressHUDUndefinedProgress) {
    ...
    // Label
    if(imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + MPMProgressHUDLabelSpacing + labelHeight / 2.0f;
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    ...
    effectX.minimumRelativeValue = @(-MPMProgressHUDParallaxDepthPoints);
    effectX.maximumRelativeValue = @(MPMProgressHUDParallaxDepthPoints);
    
    UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
    effectY.minimumRelativeValue = @(-MPMProgressHUDParallaxDepthPoints);
    effectY.maximumRelativeValue = @(MPMProgressHUDParallaxDepthPoints);
...
#else
            // If MPMProgressHUD is used inside an app extension add it to the given view
...
- (NSDictionary*)notificationUserInfo {
    return (self.statusLabel.text ? @{MPMProgressHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
#if !defined(MPM_APP_EXTENSIONS) && TARGET_OS_IOS
    self.frame = [[[UIApplication sharedApplication] delegate] window].bounds;
    UIInterfaceOrientation orientation = UIApplication.sharedApplication.statusBarOrientation;
#elif !defined(MPM_APP_EXTENSIONS) && !TARGET_OS_IOS
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:MPMProgressHUDDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:MPMProgressHUDDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak MPMProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong MPMProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.progress = MPMProgressHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)fadeIn:(id)data {
    ...
    if(self.defaultMaskType != MPMProgressHUDMaskTypeNone) {
    ...
    if(self.backgroundView.alpha != 1.0f) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:MPMProgressHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(self.backgroundView.alpha == 1.0f){
                ...
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:MPMProgressHUDDidAppearNotification
                                                                    object:self
                                                                  userInfo:[self notificationUserInfo]];
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(MPMProgressHUDDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            // Post notification to inform user
            [[NSNotificationCenter defaultCenter] postNotificationName:MPMProgressHUDWillDisappearNotification
                                                                object:nil
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(self.backgroundView.alpha == 0.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:MPMProgressHUDDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == MPMProgressHUDAnimationTypeFlat){
        // Check if spinner exists and is an object of different class
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[MPMIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[MPMIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        MPMIndefiniteAnimatedView *indefiniteAnimatedView = (MPMIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (MPMProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[MPMProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (MPMProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[MPMProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == MPMProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == MPMProgressHUDStyleDark) {
...
-(UIView *)backgroundView {
    ...
    // Update styling
    if(self.defaultMaskType == MPMProgressHUDMaskTypeGradient){
        if(!_backgroundRadialGradientLayer){
            _backgroundRadialGradientLayer = [MPMRadialGradientLayer layer];
        }
        ...
        if(self.defaultMaskType == MPMProgressHUDMaskTypeBlack){
            _backgroundView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4];
        } else if(self.defaultMaskType == MPMProgressHUDMaskTypeCustom){
...
- (void)fadeInEffects {
    if(self.defaultStyle != MPMProgressHUDStyleCustom) {
        // Add blur effect
        UIBlurEffectStyle blurEffectStyle = self.defaultStyle == MPMProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleLight;
...
- (void)setDefaultAnimationType:(MPMProgressHUDAnimationType)animationType {
