...
typedef void(^CompleteBlock)(NSArray *cycle);
...
- (void)sure:(UIButton *)sender {
    if (self.cycleArray.count == 0) {
        [MPMProgressHUD showErrorWithStatus:@"请选择考勤日期"];
        return;
    }
    NSArray *daysOfWeek = [self translateDaysIndexToArray:self.cycleArray];
    if (self.completeBlock) {
        self.completeBlock(daysOfWeek);
    }
    [self dismiss];
}
...
- (NSArray *)translateDaysIndexToArray:(NSArray<NSIndexPath *> *)arr {
    NSString *cycle = @"";
