...
- (instancetype)convertModelWithDictionary:(NSDictionary *)dic {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if ([value isKindOfClass:[NSArray class]]) {
            // 在model里面定义 参数名_type 的方法，在这个方法返回这个NSArray的数据的model
            if ([self respondsToSelector:NSSelectorFromString([NSString stringWithFormat:@"%@_type",key])]) {
                // 转换成id类型，就能调用任何对象的方法
                id idSelf = self;
                // 获取数组中字典对应的模型
                NSString *modelName = [idSelf performSelector:NSSelectorFromString([NSString stringWithFormat:@"%@_type",key])];
                // 生成模型
                Class classModel = NSClassFromString(modelName);
                NSMutableArray *arrM = [NSMutableArray array];
                // 遍历字典数组，生成模型数组
                for (NSDictionary *dict in value) {
                    // 字典转模型
                    id model = [[[classModel alloc] init] convertModelWithDictionary:dict];
